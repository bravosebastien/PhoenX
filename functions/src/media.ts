...
export const getInheritedFileUrl = onCall(async (request) => {
    ...
    const ALLOWED_TYPES = ["entries", "standaloneMedia", "book", "persons", "personMedia", "recipients", "witnesses", "depositaries", "personalities", "personalityMedia", "rankings", "rankMedia", "legacyVideo"];
    ...
    const docRef = docType === "personMedia"
        ...
        : docType === "rankMedia"
        ? db.collection("users").doc(creatorId).collection("rankings").doc(request.data.rankingId).collection("rankMedia").doc(docId)
        : docType === "legacyVideo"
        ? db.collection("users").doc(creatorId).collection("legacyVideo").doc(docId)
    ...
    if (field === "coverUrl" || field === "coverImageUrl") {
        ...
        else if (docType === "rankMedia") storageUrl = itemData.mediaPath;
        else if (docType === "legacyVideo") storageUrl = itemData.videoPath;
