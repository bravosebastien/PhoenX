...
fun com.example.phoenx.data.local.PersonEntity.toFirestoreMap(storageUrl: String? = null, encounterStorageUrl: String? = null): Map<String, Any?> {
    return mapOf(
        ...
        "relationshipDetail" to relationshipDetail,
        "characterType" to characterType,
        "manualGenerationOffset" to manualGenerationOffset,
...
fun DocumentSnapshot.toPersonEntity(): com.example.phoenx.data.local.PersonEntity {
    ...
    return com.example.phoenx.data.local.PersonEntity(
        ...
        weight = getLong("weight")?.toInt(),
        eyeColor = getString("eyeColor"),
        hairColor = getString("hairColor"),
        clothingStyle = getString("clothingStyle"),
        profession = getString("profession"),
        hasChildren = getBoolean("hasChildren"),
        relationshipDetail = getString("relationshipDetail"),
        characterType = getString("characterType") ?: "HUMAN",
        manualGenerationOffset = getLong("manualGenerationOffset")?.toInt() ?: 0,
        // v9.4.24: Genealogy fields
...
fun Map<String, Any?>.toOfflineEntry(encryptionManager: EncryptionManager, explicitKey: ByteArray? = null): OfflineEntry? {
    ...
    return OfflineEntry(
        ...
        userTitle = finalUserTitle,
        scheduledTimestamp = this["scheduledTimestamp"].asLong(),
        unlockAfterDays = this["unlockAfterDays"].asLong()?.toInt() ?: 30,
