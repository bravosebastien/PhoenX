...
- **Régénération partielle du Livre — chantier détaillé en section 8bis.** Lot 0 et Lot 1 clos et commités (3 septembre), ainsi qu'un diagnostic complémentaire hors plan qui a corrigé l'ordre chronologique des chapitres (par le code, pas par simple consigne IA), ajouté une consigne de prudence factuelle à l'IA pour les souvenirs pauvres en contenu, et fiabilisé le rattachement chapitre/`sceneIds`. Lots 2/3/4 non commencés — voir 8bis pour l'état précis et les pièges impératifs du Lot 3.
...
- **Diagnostic `authorProfile` non daté (3 septembre, ✅ CLOS, correctif léger déployé) :** observation du Créateur — les infos du Portrait de Vie (métier, fratrie, enfants, loisirs, portrait physique) se concentraient dans le chapitre "aujourd'hui" plutôt que d'être réparties par période de vie. Diagnostic : `CreatorProfileEntity` est un objet plat et **non daté** (aucun champ de période), contrairement à l'Arbre où c'est le tri par `age` de chaque SCÈNE (pas la fiche personne elle-même) qui crée l'effet chronologique. `generateBookChapters` est un seul appel IA produisant tous les chapitres d'un coup ; `authorProfile` y est injecté une seule fois, en bloc global, sans aucune consigne sur la validité temporelle des faits. Une vraie refonte par période (enfance/ado/adulte) exigerait un nouveau schéma Room + migration + refonte de `CreatorRichProfileScreen.kt` + impact sur les 2 AUTRES consommateurs du même objet (`EssencePortraitViewModel`, `PreviewViewModel`) — **chantier à part, volontairement non lancé, à cadrer séparément si le besoin se confirme.** Correctif léger retenu à la place : consigne ajoutée dans le prompt de `generateBookChapters` (`ai.ts`, bloc `authorProfileInstruction`) précisant que le profil décrit l'état ACTUEL de l'auteur et ne doit pas être plaqué sur des chapitres d'un autre âge de sa vie (sauf trait de fond plausible de longue date, à utiliser sans le dater). Déployé, pas de test de régénération dédié demandé — vérification prévue au prochain vrai test du Livre.
...
- `CreatorProfileEntity` (Portrait de Vie) est un objet plat, **non daté par période de vie** (voir section 8bis, diagnostic `authorProfile` du 3 septembre) — un correctif léger de prompt a été fait, mais une éventuelle refonte par période (enfance/ado/adulte) reste un vrai chantier non lancé : nouveau schéma Room + migration + refonte de `CreatorRichProfileScreen.kt` + impact sur `EssencePortraitViewModel` et `PreviewViewModel`, autres consommateurs du même objet.
