...
- **Lot 1 — 🟡 CODÉ ET COMPILÉ, PAS ENCORE COMMITÉ NI TESTÉ SUR APPAREIL.** Modifications non commitées dans `BookModels.kt` et `BookGeneratorService.kt` : ajout de `sceneIds`/`sourceFingerprint` par chapitre, `computeChapterFingerprint()` en SHA-256, correctif annexe `plan ?: generateBookPlan()` → `effectivePlan`. Ne change rien visuellement — on arrête juste de jeter l'information. Aucune migration Room nécessaire (tout se joue dans le document Firestore du Livre). **Selon la règle du projet, ce lot doit être testé sur un vrai appareil ET commité avant d'ouvrir un nouveau chantier — y compris avant de traiter le bug de ponctuation ou tout autre point de la section 8.**
...
- **Lot 3 — non commencé, contient TOUS les pièges critiques de ce chantier (voir ci-dessous) — à tester avec une exigence particulière.** Le bouton n'envoie plus que les chapitres cochés + plan réduit.
